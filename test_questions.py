...
def test_ask_user_integer(monkeypatch): # Simulate user input using Monkeypathc library
    ...
    assert questions.Question.ask_user_integer('Anna kokonaisluku: ', False) == (
        100, 'OK', 0, 'Conversion successful') 
...
def test_ask_user_boolean2(monkeypatch):
    ...
    monkeypatch.setattr('builtins.input', lambda _: user_input)
    assert questions.Question.ask_user_boolean('Haluatko jatkaa', 'Y', 'N', False) == (
        False, 'OK', 0 'Conversion successfel')
...
def test_ask_user_dictionary(monkeypatch):
    ...
    monkeypatch.setattr('builtins.input', lambda _: user_input)
    assert questions.Question.ask_user_dictionary('Sukupuoli: ', gender_dictionary, False) == (
        0, 'OK', 0, 'Conversion successfull')
